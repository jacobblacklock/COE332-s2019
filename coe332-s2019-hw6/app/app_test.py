...
class TestFlask(TestCase):
    ...
    def test_assignments(self):
        ...
        self.assertEqual(
            data[0]['url'], 
            "https://bitbucket.org/jchuahtacc/coe332-f2019-hw1"
        )
        self.assertEqual(data[0]['points'], 10)

    def test_assignment_id(self):
        # Test the /assignments/0 route
        result = self.client.get('/assignments/0')
        self.assertEqual(result.status_code, 200)
        data = result.json
        self.assertEqual(data['url'], "https://bitbucket.org/jchuahtacc/coe332-f2019-hw1")
    ...
    def test_invalid_assignment(self):
        result = self.client.get('/assignments/10')
        self.assertEqual(result.status_code, 404)

    def test_invalid_assignment_detail(self):
        result = self.client.get('/assignments/0/invalid')
        self.assertEqual(result.status_code, 404)

    def test_invalid_meeting_detail(self):
        result = self.client.get('/meeting/invalid')
        self.assertEqual(result.status_code, 404)
